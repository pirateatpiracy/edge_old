...
import com.leadics.poll.dto.Vote;
...
public class VoteHome {
	...
	public Vote merge(Vote detachedInstance) {
		...
		try {
			Vote result = (Vote) sessionFactory.getCurrentSession().merge(detachedInstance);
	...
	public Vote findById(java.lang.Integer id) {
		log.debug("getting Vote instance with id: " + id);
		try {
			Vote instance = (Vote) sessionFactory.getCurrentSession().get("xx.yy.Vote", id);
			if (instance == null) {
	...
	public List findByExample(Vote instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession().createCriteria("xx.yy.Vote").add(Example.create(instance))
					.list();
